## KEL notes
- Maybe separate into 3 separate posts, one for each site
- For the first set of temp graphs, we probably want to take the max and min per month (instead of mean_yearly_min)
- Certain values should have NAs if they can't be calculated (e.g. dd0)
- Seems that Hog Island vs. Dari. is different
- I think for these we will need to pool all data to get stats
- We'll have to talk through it
